...
const AreaPickerBottomSheet = (props, ref) => {
    ...
    const fetchBrandListFromServer = () => {
        ...
        try {
            ApiClient.post(`${ApiRoute.BASE_URL}${ApiRoute.GET_AREAS_BY_CITYID}`, data)
                .then((response) => {
    ...
    const renderListItem = ({ item, index }) => (
        <TouchableOpacity style={styles.item} onPress={() => {
            ...
            <Text style={[styles.title, selectedID === item.area_ID ? { fontWeight: 'bold' } : { fontWeight: 'normal' }]}>{item?.areaName}</Text>
            <Image source={selectedID === item.area_ID ? ic_checked_radio : ic_unchecked_radio} style={styles.radioIcon} />
        </TouchableOpacity>
    );
        ...
        return (
            <SafeAreaView style={{ flex: 1 }}>
                <View style={styles.container}>
                    <Text style={styles.header}>Select Area</Text>
                    <FlatList
                        data={data}
    ...
    return (
        <View style={styles.containerMain}>
            ...
            <TouchableOpacity style={styles.fieledContainer}
                ...
                <Text style={styles.label}>{selectedValue !== '' ? selectedValue : 'Select Area'}</Text>
...
export default forwardRef(AreaPickerBottomSheet);