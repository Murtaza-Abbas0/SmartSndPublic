...
const SubmitOrderListItem = ({ item, onChangeQuantity }) => {
  ...
  const handleAmount = (val) => {
    val === "" ? setTotalQuantity(0) : setTotalQuantity(parseInt(val))
  ...
  useEffect(() => {
    ...
    onChangeQuantity({
      ...
      totalAmount: item?.perPiecePrice * totalQuantity,
      returnQuantity: 0,
  ...
  return (
    <View style={{ flex: 1 }} >
      {item?.isTargetProduct == 1 ?
        <View>
          <View style={styles.itemsContainerForAssigned} >
            <Text style={[styles.brandName, { color: Colors.textColor }]} >{item.name}</Text>
            <TextInput
...
export default memo(SubmitOrderListItem);
