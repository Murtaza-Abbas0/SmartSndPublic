                ...
                item.productGroupModels.map((val) => {
                    return (
                        <View style={styles.objectRowContainer} >
                            ...
                            <Text style={[styles.headerLabelListItem, { color: Colors.textColor }]}>{val.salesQuantity.toLocaleString()}</Text>
                            <Text style={[styles.headerLabelListItem, { color: Colors.textColor }]}>{val.salesInPKR.toLocaleString(undefined, { minimumFractionDigits: 2 })}</Text>
...
const TodaysBookingCountViaCategory = ({ route, navigation }) => {
    ...
    let { isTetra } = route?.params

    // console.log('isTetra: ', isTetra);
    ...
    const getTitle = () => {
        if (isTetra == 0) {
            return "Combined Daily Dashboard Details"
        } else if (isTetra == 1) {
            return "Daily Dashboard Details (Tetra)"
        } else if (isTetra == 2) {
            return "Daily Dashboard Details (Sauces)"
        }
    }
    ...
    const getOrderSummary = async (showLoadingModal, pageNo) => {
        ...
        try {
            await ApiClient.post(`${ApiRoute.BASE_URL}${ApiRoute.DAILY_SALES_REPORT_FOR_DASHBOARD}?salesmanid=${userData?.salesMan_Encrypted}&ProductTypeID=${isTetra}`, {
                headers: {
                    'Accept': 'text/plain',
                ...
                .then(async (response) => {
                    ...
                    if (response.data.isSuccess && response.data.data != null) {
                        ...
                        console.log('response.data?.data.productCategoryModels: ', response.data?.data.productCategoryModels[0]?.productGroupModels);
                        ...
                        response.data?.data.productCategoryModels.map((val) => {
                            val.productGroupModels.map((item) => {
                                ...
                                totalsSalesQuantity = item?.salesQuantity + totalsSalesQuantity
                                totalSales = item?.salesInPKR + totalSales
                                // console.log('val: ', val);
    ...
    return (
        <View style={{ flex: 1, flexDirection: 'column', backgroundColor: 'white' }}>
            <CommonHeaderWithBackButton title={getTitle()} />
            ...
            <View style={styles.bottomContainer} >
                <View style={styles.bottomHeadingContainer} >
                    ...
                    <Text style={styles.bottomHeadingText} >Total Sales Amount</Text>
                </View>
                <View style={styles.bottomValuesContainer} >
                    ...
                    <Text style={styles.bottomHeadingText} >{totalSalesAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</Text>
                </View>
            </View>
...
export default TodaysBookingCountViaCategory;